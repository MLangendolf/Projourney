  ...
  const handleAreaToggle = (areaId: string) => {
    setFormData((prev) => ({
      ...
      areasInteresse: prev.areasInteresse.includes(areaId)
        ? prev.areasInteresse.filter((id) => id !== areaId)
  ...
  const handleLanguageToggle = (language: string) => {
    setFormData((prev) => ({
      ...
      linguagens: prev.linguagens.includes(language)
        ? prev.linguagens.filter((lang) => lang !== language)
                ...
                <div className="bg-gray-800/50 p-4 rounded-lg">
                  ...
                  <div className="space-y-2">
                    <p className="text-gray-300">
                      ...
                      {experienceLevels.find((l) => l.value === formData.experiencia)?.label || "Não informado"}
                    </p>
                    {formData.areasInteresse.length > 0 && (
                      <div>
                        <strong className="text-gray-300">Áreas de interesse:</strong>
                        <div className="flex flex-wrap gap-2 mt-1">
                          {formData.areasInteresse.map((areaId) => {
                            const area = techAreas.find((a) => a.id === areaId)
                            return area ? (
